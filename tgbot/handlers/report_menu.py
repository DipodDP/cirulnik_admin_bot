...
async def complete_report(message: types.Message, state: FSMContext, config: Config):
    ...
    if state_data["daytime"] == "morning":
        await on_report(
            ...
            ReportBuilder(state_data).construct_morning_report(),
        )
        await broadcast_messages(config.tg_bot.admin_ids, state_data["open_check"])
        await broadcast_messages(
            config.tg_bot.admin_ids, state_data["solarium_counter"]
        ) if "solarium_counter" in state_data else ...
    elif state_data["daytime"] == "evening":
        await on_report(
            message.bot,
            config.tg_bot.admin_ids,
            ReportBuilder(state_data).construct_evening_report(),
        )
